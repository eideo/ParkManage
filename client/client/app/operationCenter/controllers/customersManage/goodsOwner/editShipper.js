...
app.controller('editShipperCtrl', ['$scope', '$http', 'dialog', 'customersManageService','Upload', function ($scope, $http, dialog, customersManageService,Upload) {
    var vm = this;
    ...
    vm.edit = function(){
       ...
       // vm.jsondata.orgcodelicurl=$scope.upload($scope.filesorg);
        if($scope.myForm.$valid){
            customersManageService.editEnterpriseInfo(vm.jsondata).then(function (response) {
                if(response.data.code == "200"){
     ...
     $scope.upload = function (files,callback) {
        if (files && files.length) {
            for (var i = 0; i < files.length; i++) {
                var file = files[i];
                Upload.upload({
                    url: lpt_host + '/zeus/ws/system/fastdfs/upload',
